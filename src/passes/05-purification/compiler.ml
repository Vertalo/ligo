    ...
    (fun (decl_var,free_var : O.expression_variable list * O.expression_variable list) (ass_exp : O.expression) ->
      match ass_exp.expression_content with
        | E_let_in {let_binder;mut=false;rhs;let_result} ->
          let {var;_} : _ O.binder = let_binder in
          ok (true,(var::decl_var, free_var),O.e_let_in let_binder false false rhs let_result)
        | E_let_in {let_binder;mut=true; rhs;let_result} ->
          let {var;_} : _ O.binder = let_binder in
          if List.mem ~compare:compare_var var decl_var then
            ok (true,(decl_var, free_var), O.e_let_in let_binder false false rhs let_result)
          else(
            let free_var = if (List.mem ~compare:compare_var var free_var) then free_var else var::free_var in
            let expr = O.e_let_in_ez (env,O.t_wildcard ()) false false (O.e_update (O.e_variable env) [O.Access_record (Var.to_name var.wrap_content)] (O.e_variable var)) let_result in
    ...
    (fun (decl_var,free_var : O.expression_variable list * O.expression_variable list) (ass_exp : O.expression) ->
      ...
      match ass_exp.expression_content with
        | E_let_in {let_binder;mut=false;} ->
          let {var;ty=_} : _ O.binder = let_binder in
        ...
        | E_let_in {let_binder;mut=true; rhs;let_result} ->
          ...
          if List.mem ~compare:compare_var var decl_var then
            ...
            let free_var =
              if (List.mem ~compare:compare_var var free_var)
              then free_var
              else var::free_var in
            let expr = O.e_let_in_ez (env,O.t_wildcard ()) false false (
              O.e_update (O.e_variable env) [O.Access_tuple Z.zero; O.Access_record (Var.to_name var.wrap_content)] (O.e_variable var)
              )
              let_result in
            ok (true,(decl_var, free_var), O.e_let_in let_binder false  false rhs expr)
          )
...
and restore_mutable_variable (expr : O.expression->O.expression) (free_vars : O.expression_variable list) (env : O.expression_variable) =
  let aux (f: O.expression -> O.expression) (ev: O.expression_variable) =
    fun expr -> f (O.e_let_in_ez (ev,O.t_wildcard ()) true false (O.e_accessor (O.e_variable env) [O.Access_record (Var.to_name ev.wrap_content)]) expr)
  in
...
let rec compile_type_expression : I.type_expression -> (O.type_expression,Errors.purification_error) result =
  ...
  match te.type_content with
    ...
    | I.T_constant (TC_michelson_or, [l;r]) ->
      ...
      let%bind (l,r) = bind_map_pair self (l,r) in
...
and compile_expression' : I.expression -> (O.expression option -> O.expression, Errors.purification_error) result =
  ...
  match e.expression_content with
    ...
    | I.E_let_in {let_binder;inline;rhs;let_result} ->
      ...
      return @@ O.E_let_in {let_binder;mut=false; inline; rhs; let_result}
    | I.E_raw_code rc ->
    ...
    | I.E_cond {condition;then_clause;else_clause} ->
      ...
      if (List.length free_vars != 0) then 
        ...
        let return_expr = fun expr ->
          O.e_let_in_ez (env,O.t_wildcard ()) false false (store_mutable_variable free_vars) @@
          O.e_let_in_ez (env,O.t_wildcard ()) false false cond_expr @@
          expr 
        in
    ...
    | I.E_assign {variable; access_path; expression} ->
      ...
      ok @@ fun expr ->
        O.e_let_in_ez ~loc (variable,O.t_wildcard ()) true false rhs
  ...
  match cases with 
    | I.Match_option {match_none;match_some} ->
      ...
      if (List.length free_vars != 0) then
        ...
        let return_expr = fun expr ->
          O.e_let_in_ez (env,O.t_wildcard ()) false false (store_mutable_variable free_vars) @@
          O.e_let_in_ez (env,O.t_wildcard ()) false false match_expr @@
          expr 
        in
        ok @@ restore_mutable_variable return_expr free_vars env
      else
...
and compile_while I.{cond;body} =
  ...
  let aux name expr=
    O.e_let_in_ez (name,O.t_wildcard ()) false false (O.e_accessor (O.e_variable binder) [Access_tuple Z.zero; Access_record (Var.to_name name.wrap_content)]) expr
  in
  ...
  let aux_func = 
    O.e_lambda_ez (binder, O.t_wildcard ()) @@ 
    restore @@
  ...
  let return_expr = fun expr -> 
    O.e_let_in_ez let_binder false false init_rec @@
    O.e_let_in_ez let_binder false false loop @@
    O.e_let_in_ez let_binder false false (O.e_accessor (O.e_variable env_rec) [Access_tuple Z.zero]) @@
    expr
  in
...
and compile_for I.{binder;start;final;incr;f_body} =
  ...
  let ctrl = 
    O.e_let_in_ez (binder,O.t_int ()) false false (O.e_constant C_ADD [ O.e_variable binder ; step ]) @@
    O.e_let_in_ez (env_rec, O.t_wildcard ()) false false (O.e_update (O.e_variable env_rec) [Access_tuple Z.one] @@ O.e_variable binder)@@
    continue_expr
  in
  ...
  let aux name expr=
    O.e_let_in_ez (name,O.t_wildcard ()) false false (O.e_accessor (O.e_variable env_rec) [Access_tuple Z.zero; Access_record (Var.to_name name.wrap_content)]) expr
  in
  ...
  let aux_func = O.e_lambda_ez (env_rec,O.t_wildcard ()) @@ 
                 O.e_let_in_ez (binder,O.t_int ()) false false (O.e_accessor (O.e_variable env_rec) [Access_tuple Z.one]) @@
  ...
  let let_binder = (env_rec,O.t_wildcard ()) in
  let return_expr = fun expr -> 
    O.e_let_in_ez (binder, O.t_int ()) false false start @@
    O.e_let_in_ez let_binder false false init_rec @@
    O.e_let_in_ez let_binder false false loop @@
    O.e_let_in_ez let_binder false false (O.e_accessor (O.e_variable env_rec) [Access_tuple Z.zero]) @@
    expr
...
and compile_for_each I.{fe_binder;collection;collection_type; fe_body} =
  ...
  let aux name expr=
    O.e_let_in_ez (name,O.t_wildcard ()) false false (O.e_accessor (O.e_variable args) [Access_tuple Z.zero; Access_record (Var.to_name name.wrap_content)]) expr
  in
  ...
  let restore = match collection_type with
    | Map -> (match snd fe_binder with 
      | Some v -> fun expr -> restore (O.e_let_in_ez (fst fe_binder, O.t_wildcard ()) false false (O.e_accessor (O.e_variable args) [Access_tuple Z.one; Access_tuple Z.zero]) 
                                    (O.e_let_in_ez (v, O.t_wildcard ()) false false (O.e_accessor (O.e_variable args) [Access_tuple Z.one; Access_tuple Z.one]) expr))
      | None -> fun expr -> restore (O.e_let_in_ez (fst fe_binder, O.t_wildcard ()) false false (O.e_accessor (O.e_variable args) [Access_tuple Z.one; Access_tuple Z.zero]) expr) 
    )
    | _ -> fun expr -> restore (O.e_let_in_ez (fst fe_binder, O.t_wildcard ()) false false (O.e_accessor (O.e_variable args) [Access_tuple Z.one]) expr)
  in
  let lambda = O.e_lambda_ez (args,O.t_wildcard ()) (restore for_body) in
  let%bind op_name = match collection_type with
   | Map -> ok @@ O.C_MAP_FOLD | Set -> ok @@ O.C_SET_FOLD | List -> ok @@ O.C_LIST_FOLD 
  in
  let fold = fun expr -> 
    O.e_let_in_ez (env_rec,O.t_wildcard ()) false false (O.e_constant op_name [lambda; collect ; init_record]) expr
  in
